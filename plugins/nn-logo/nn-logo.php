 ...
 */
function create_block_nn_logo_block_init() {
	/**
...
add_action( 'init', 'create_block_nn_logo_block_init' );


function wpdocs_add_new_block_category( $block_categories, $block_editor_context ) {
	// Check if we are in the block editor context
	if ( $block_editor_context == 'editor' ) {
		return $block_categories;
	}
	// Add your custom category at the top
	array_unshift( $block_categories, array(
		'slug'  => 'nn-blocks', // Unique slug for the category
		'title' => __( 'NN Blocks' ), // Title for the category
	) );
    return $block_categories;
}
